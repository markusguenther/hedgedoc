...
describe('RealtimeNoteService', () => {
  ...
  let getLatestRevisionFunctionMock: jest.Mock;
  let mockedNote: Note;
  ...
  let latestRevisionExists = true;

  beforeEach(async () => {
    jest.resetAllMocks();
    jest.resetModules();

    getLatestRevisionFunctionMock = jest.fn();
    const revisionsService = Mock.of<RevisionsService>({
      getLatestRevision: getLatestRevisionFunctionMock,
    });
